...
class Round(commands.Cog):
    ...
    @round.command(
        name="challenge", brief="Challenge multiple users to a round"
    )
    ...
    async def _invalidate(self, ctx, member: discord.Member):
        ...
        if not self.db.in_a_round(ctx.guild.id, member.id):
            await discord_.send_message(
                ctx, f"{member.mention} is not in a round"
            )
            return
    ...
    async def update(self, ctx):
        ...
        for round in rounds:
            try:
                ...
                if resp[1]:
                    round_info = self.db.get_round_info(
                        round.guild, round.users
                    )
                    ...
                    eloChanges = elo.calculateChanges(
                        [
                            [
                                (
                                    await discord_.fetch_member(
                                        ctx.guild, user.id
                                    )
                                ),
                                user.rank,
                                self.db.get_match_rating(
                                    round_info.guild, user.id
                                )[-1],
                            ]
                    ...
                    if round_info.tournament == 1:
                        tournament_info = self.db.get_tournament_info(
                            round_info.guild
                        )
                        ...
                        if ranklist[1].rank == 1 and tournament_info.type != 2:
                            ...
                            if not res[0]:
                                ...
                                if await tournament_helper.validate_tournament_completion(
                                    ...
                                    winner_handle = (
                                        await tournament_helper.get_winner(
    ...
    @round.command(
        name="custom", brief="Challenge to a round with custom problemset"
    )
