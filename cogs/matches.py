...
async def plot_graph(ctx, data, handle):
    ...
    embed.set_footer(
        text="Requested by " + str(ctx.author), icon_url=ctx.author.avatar_url
    )
...
class Matches(commands.Cog):
    ...
    async def _invalidate(self, ctx, member: discord.Member):
        ...
        await ctx.send(
            embed=discord.Embed(
                description="Match has been invalidated",
    ...
    async def invalidate(self, ctx):
        ...
        opponent = await discord_.fetch_member(
            ctx.guild,
            match_info.p1_id
            if match_info.p1_id != ctx.author.id
            else match_info.p2_id,
        )
        await ctx.send(
        ...
        try:
            ...
            await ctx.send(
                f"{ctx.author.mention} your opponent didn't respond in time"
            )
    ...
    async def recent(self, ctx, member: discord.Member = None):
        data = self.db.get_recent_matches(
            ctx.guild.id, member.id if member else None
        )
        if len(data) == 0:
                ...
                if resp[1]:
                    ...
                    ranklist.append(
                        [
                            await discord_.fetch_member(ctx.guild, match.p1_id),
                            p1_rank,
                            self.db.get_match_rating(ctx.guild.id, match.p1_id)[
                                -1
                            ],
                        ]
                    )
                    ranklist.append(
                        [
                            await discord_.fetch_member(ctx.guild, match.p2_id),
                            p2_rank,
                            self.db.get_match_rating(ctx.guild.id, match.p2_id)[
                                -1
                            ],
                        ]
                    )
    ...
    async def rating(self, ctx, member: discord.Member = None):
        ...
        if len(data) <= 1:
            ...
            return
        await plot_graph(
            ctx, data[1:], self.db.get_handle(ctx.guild.id, member.id)
        )
    ...
    async def ranklist(self, ctx):
        ...
        if len(res) == 0:
            await discord_.send_message(
                ctx, "No user has played a match so far"
            )
            return
        ...
        for x in res:
            try:
                data.append(
                    [
                        (await discord_.fetch_member(ctx.guild, x[0])).name,
                        str(x[1]),
                    ]
                )
            except Exception:
