...
class Handles(commands.Cog):
    ...
    @handle.command(
        brief="Set someone's handle (Admin/Mod/Lockout Manager only)"
    )
    async def set(self, ctx, member: discord.Member, handle: str):
        ...
        handles = list(
            filter(
                lambda x: x[2] == handle, self.db.get_all_handles(ctx.guild.id)
            )
        )
        if len(handles):
    ...
    @handle.command(
        brief="Remove someone's handle (Admin/Mod/Lockout Manager only)"
    )
    async def remove(self, ctx, member: discord.Member):
        ...
        if self.db.in_a_round(ctx.guild.id, member.id) or self.db.in_a_match(
            ...
            await discord_.send_message(
                ctx,
                f"{member.mention} is currently in a match/round. Try again later",
            )
            return
    ...
    async def identify(self, ctx, handle: str):
        ...
        if res != await self.cf.get_first_name(handle):
            await discord_.send_message(
                ctx,
                f"Unable to set handle, please try again {ctx.author.mention}",
            )
            return
        ...
        for x in data:
            try:
                data1.append(
                    [
                        (
                            await discord_.fetch_member(ctx.guild, int(x[1]))
                        ).name,
                        x[2],
                        x[3],
                    ]
