...
class Tournament(commands.Cog):
    ...
    def make_tournament_embed(self, ctx):
        ...
        embed.set_author(
            name="Lockout commands help", icon_url=ctx.me.avatar_url
        )
        embed.set_footer(
    ...
    async def register(self, ctx):
        ...
        if not self.db.get_handle(ctx.guild.id, ctx.author.id):
            await discord_.send_message(
                ctx,
                "Your handle is not set, set your handle first and try again",
            )
            return
        ...
        if len(registrants) == MAX_REGISTRANTS:
            await discord_.send_message(
                ctx,
                "The tournament has already reached its max registrants limit!",
            )
            return
    ...
    @tournament.command(
        name="unregister", brief="Unregister from the tournament"
    )
    ...
    @tournament.command(
        name="_unregister",
        brief="Forcefully unregister someone from the tournament",
    )
    ...
    @tournament.command(
        name="info", brief="Get basic information about the tournament"
    )
    async def info(self, ctx):
        ...
        desc += (
            f"**Registrations**: {len(self.db.get_registrants(ctx.guild.id))}\n"
        )
    ...
    @tournament.command(
        name="begin", brief="Begin the tournament", aliases=["start"]
    )
    ...
    async def begin(self, ctx):
        ...
        if tournament_info.status == 0:
            ...
            desc += (
                f"`.tournament info`: View general info about the tournament\n"
            )
    ...
    @tournament.command(
        name="forcedraw", brief="Force draw a match (Swiss only)"
    )
    @commands.cooldown(1, 10, BucketType.user)
    ...
    async def match_invalidate(self, ctx, idx: int):
        ...
        if match_id:
            ...
            await discord_.send_message(
                ctx, f"Couldn't find match number `{idx}`"
            )
    ...
    async def recent(self, ctx):
        ...
        if not data:
            await discord_.send_message(
                ctx, "No tournaments have been played so far"
            )
            return
