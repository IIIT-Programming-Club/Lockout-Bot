...
class Paginator:
    ...
    def get_page_elements(self, page_no):
        return self.data[
            (page_no - 1) * self.per_page : page_no * self.per_page
        ]
    ...
    def get_page_content(self, page_no):
        ...
        for i in range(len(val)):
            val[i] = max(
                max([len(element[i]) for element in elements]),
                len(self.headers[i]),
            )
        content, dashes = "", ""
        ...
        for i in range(len(val)):
            content += (
                self.headers[i] + " " * (val[i] - len(self.headers[i])) + " "
            )
            dashes += "-" * val[i] + " "
        ...
        for i in range(len(elements)):
            for j in range(len(val)):
                content += (
                    elements[i][j] + " " * (val[j] - len(elements[i][j])) + " "
                )
            content += "\n"
        return f"```\n{content}```"
