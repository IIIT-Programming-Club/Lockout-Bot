...
async def is_a_match(
    ...
    if not matches or "error" in matches:
        return False
    p1_id, p2_id = db.get_challonge_id(guild, p1_id), db.get_challonge_id(
        guild, p2_id
    )
...
async def validate_match(
    ...
    if not tournament_info or tournament_info.status != 2:
        return [
            False,
            "Couldn't validate tournament match: Tournament not found",
        ]

    matches = await api.get_tournament_matches(tournament_info.id)
    if not matches:
        return [False, "Couldn't validate tournament match: API Error"]
    p1_cid, p2_cid = db.get_challonge_id(guild, p1_id), db.get_challonge_id(
        guild, p2_id
    )
    ...
    return [
        False,
        "Couldn't validate tournament match: Unable to find the match",
    ]
