...
async def update_match(match_info):
    ...
    for i in range(5):
        ...
        time1, time2 = codeforces.get_solve_time(
        ...
        if (
            time1 < time2
            and time1 <= match_info.time + 60 * match_info.duration
        ):
            updates.append([i + 1, [match_info.p1_id]])
            new_status += "1"
        elif (
            time2 < time1
            and time2 <= match_info.time + 60 * match_info.duration
        ):
...
async def update_round(round_info):
    ...
    subs = [
        await cf.get_user_problems(handle, RECENT_SUBS_LIMIT)
    ...
    if updated:
        db.update_round_status(
            round_info.guild, users[0], status, problems, timestamp
        )
