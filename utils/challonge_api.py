...
class ChallongeAPI:
    ...
    async def bulk_add_participants(self, tournament_id, participants):
        url = (
            BASE_URL + f"tournaments/{tournament_id}/participants/bulk_add.json"
        )

        params = {"api_key": self.api_key, "participants": participants}
        return await self.api_response("POST", url, params)

    async def delete_tournament(self, tournament_id):
        url = (
            BASE_URL
            + f"tournaments/{tournament_id}.json?api_key={self.api_key}"
        )
        await self.api_response("DELETE", url)

    async def open_for_predictions(self, tournament_id):
        url = (
            BASE_URL + f"tournaments/{tournament_id}/open_for_predictions.json"
        )
    ...
    async def get_tournament_matches(self, tournament_id):
        url = (
            BASE_URL
            + f"tournaments/{tournament_id}/matches.json?api_key={self.api_key}"
        )
    ...
    async def get_particiapnts_info(self, tournament_id):
        url = (
            BASE_URL
            + f"tournaments/{tournament_id}/participants.json?api_key={self.api_key}"
        )
        return await self.api_response("GET", url)

    async def post_match_results(
        self, tournament_id, match_id, scores, winner_id
    ):
